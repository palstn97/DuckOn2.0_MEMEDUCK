...
const LiveRoomPage = () => {
  ...
  // 퀴즈 질문 문구 (기본값까지 i18n)
  const quizQuestion =
    entryQuestion && entryQuestion !== DEFAULT_QUIZ_PROMPT
      ? entryQuestion
      : t("live.quiz.defaultPrompt", DEFAULT_QUIZ_PROMPT);
  ...
  if (!room) {
    return (
      <>
        ...
        <KickedInfoModal
          isOpen={kickedOpen}
          title={t("live.kickedInfo.title", "입장 불가")}
          description={t(
            "live.kickedInfo.description",
            "해당 방에서 강퇴되어 입장이 불가합니다."
          )}
          confirmText={t("live.kickedInfo.confirm", "확인")}
          onConfirm={() => {
            setKickedOpen(false);
            if (artistSlug) {
              navigate(`/artist/${artistSlug}`);
  ...
  const renderVideoForWeb = () => (
    <div className="w-full max-w-full max-h-full aspect-video rounded-lg border border-gray-800 overflow-hidden">
      {stompClient ? (
        ...
        <div className="w-full h-full flex items-center justify-center text-gray-400">
          {t("live.player.connecting", "플레이어 연결 중...")}
        </div>
      )}
    </div>
  );
    ...
    >
      ...
      {isKicked ? (
        ...
        <>
          ...
          <ConfirmModal
            isOpen={isDeleteOpen}
            title={t("live.deleteRoom.title", "방 삭제")}
            description={t(
              "live.deleteRoom.description",
              "정말 방을 삭제하시겠습니까?"
            )}
            confirmText={t("live.deleteRoom.confirm", "삭제")}
            cancelText={t("live.deleteRoom.cancel", "취소")}
            onConfirm={handleDeleteRoom}
            onCancel={() => setIsDeleteOpen(false)}
          />
        </>
