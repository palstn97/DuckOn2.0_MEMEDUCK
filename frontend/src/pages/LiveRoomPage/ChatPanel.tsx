...
//       // 1) 내가 로그인 상태이고, 서버가 senderId까지 돌려줬다면 → 정확 매칭
//       if (
//         pending.self &&
//         last &&
//         myUser?.userId &&
...
//         pendingSendRef.current = null;
//       }
//       // 2) 그게 아니더라도, 메시지 개수가 늘었으면(=뭔가 들어왔으면) 보냈다고 본다
...
//     if (isRateLimitedNow) {
//       // 이미 막혀 있으면 시간만 리셋해서 계속 보이게
//       triggerRateLimited();
//       return;
//     }

//     const sentAt = Date.now();
//     // ✅ 로그인/게스트 모두 여기서 pending 설정 (다만 self 플래그만 다름)
...
//     // 2) 200ms 안에 메시지 개수가 그대로면 → 서버가 안 받아준 걸로 간주 (게스트 포함)
//     setTimeout(() => {
//       const pendingNow = pendingSendRef.current;
//       if (!pendingNow) return;
//       // 아직도 내가 200ms 전에 보낸 그 메시지라면
//       if (
  ...
  return (
    <>
      ...
      <div className="relative flex flex-col h-full bg-gray-800 text-white">
        ...
        {/* 👇 이 컴포넌트 안에서만 먹는 스크롤바 스타일 */}
        <style>{`
          .${SCROLL_CLASS} {
            scrollbar-width: thin;
            scrollbar-color: rgba(148,163,184,.35) rgba(15,23,42,0);
          }
          ...
          .${SCROLL_CLASS}:hover::-webkit-scrollbar-thumb {
