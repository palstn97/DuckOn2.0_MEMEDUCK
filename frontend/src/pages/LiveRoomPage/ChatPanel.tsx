...
//         {lastUnread && !isAtBottomRef.current && (
//           <div
//             onClick={() => {
//               setLastUnread(null);
...
//           >
//             <div className="bg-white rounded-full shadow-lg px-5 py-2.5 hover:bg-gray-50 transition-colors">
//               <div className="flex items-center gap-1.5 text-sm">
//                 <span className="font-semibold text-gray-900 whitespace-nowrap">
//                   {lastUnread.senderNickName}
//                 </span>
//                 <span className="text-gray-700 truncate max-w-[160px]">
//                   {lastUnread.content.length > 10 
//                     ? `${lastUnread.content.slice(0, 10)}...` 
//                     : lastUnread.content}
//                 </span>
//               </div>
//             </div>
...
//   if (!isOpen) return null;
//   return (
//     <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-[300]">
//       <div className="bg-gray-700 rounded-lg p-6 shadow-xl w-full max-w-sm">
//         <h3 className="text-lg font-bold text-white">사용자 차단</h3>
//         <p className="text-sm text-gray-300 mt-2">
//           정말로 <span className="font-semibold text-purple-400">{nickname}</span>님을
...
//   const listRef = useRef<HTMLDivElement | null>(null);
//   const endRef = useRef<HTMLDivElement | null>(null);
//   const isAtBottomRef = useRef(true); // 논리용 ref
//   const prevLenRef = useRef(0);

//   // 렌더와 동기화되는 '바닥 여부'
//   const [atBottom, setAtBottom] = useState(true);

//   // 입력 영역(footer) 높이 측정 → 겹침 방지용
...
//   // footer 높이 추적 (입력창 높이가 변해도 리스트가 가려지지 않도록)
//   useEffect(() => {
...
//       // 시스템 메시지(ENTER)는 자동 스크롤 로직 제외
//       const isSystem = lastMsg?.chatType === "ENTER";
//       if (!lastMsg || isSystem) {
//         prevLenRef.current = messages.length;
//         return;
//       }

//       // 내가 보낸 메시지 판정(문자/숫자 혼용 대비)
//       const fromMe = String(lastMsg?.senderId ?? "") === String(myUser?.userId ?? "");
...
//           onScroll={onScroll}
//           className="relative flex-1 space-y-4 overflow-y-auto overscroll-contain p-4 min-h-0"
//           // 입력창 높이 + 여유만큼 padding-bottom → 마지막 줄이 가려지지 않게
//           style={{ paddingBottom: (footerH || 88) + 8 }}
//         >
//           {messages.map((msg, index) => {
//             if (msg.chatType === "ENTER") {
//               return (
...
//                           leaveTo="transform scale-95 opacity-0"
//                         >
//                           <Popover.Panel className="absolute z-50 top-0 left-full ml-2 w-40 bg-gray-600 border border-gray-500 rounded-lg shadow-lg">
//                             <div className="flex flex-col p-1">
//                               <button
//                                 onClick={() =>
//                                   openBlockConfirm({
