...
//    *    - 라이브 섹션은 웹처럼:
//    *      1) 상단 '라이브 방' 카드
//    *      2) 그 아래에 별도의 라이브 영상 카드(VideoCard)
//    *      3) 앱에서도 '새 방 만들기' 클릭 시 CreateRoomModal 오픈
//    * -------------------------------------------------- */
//   if (isNativeApp) {
//     return (
//       <div className="w-full min-h-screen bg-[#F8F7FF]">
...
//                   <div className="space-y-4">
//                     {liveRooms.map((room: any) => (
//                       // ✅ 웹에서 쓰는 VideoCard를 그대로 사용해서
//                       //     두 번째/세 번째 스샷 같은 카드 형태 유지
//                       <VideoCard key={room.roomId} {...room} />
//                     ))}
//                   </div>
//                 )}
...
const ArtistDetailPage = () => {
  ...
  // const handleFollowedArtistClick = (targetId: number, nameEn: string) => {
  //   const slug = createSlug(nameEn);
  //   navigate(`/artist/${slug}`, { state: { artistId: targetId } });
  // };

  /* --------------------------------------------------
   * A. 앱(Capacitor) 레이아웃
   *    - 상단 아티스트 카드 / 팬톡 카드 유지
   *    - 팔로우한 아티스트: LeftSidebar 데이터를 활용한 앱 전용 카드 추가
   *    - 라이브 섹션은 분리된 카드 + VideoCard 목록
   * -------------------------------------------------- */
  if (isNativeApp) {
    return (
      <div className="w-full min-h-screen bg-[#F8F7FF]">
        <main className="px-4 pt-4 pb-6 space-y-6">
          ...
          {/* {isLoggedIn && (
            <section className="bg-white rounded-2xl shadow-md p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold">팔로우한 아티스트</h2>
                <button
              ...
              {followedArtists.length > 0 ? (
                <div className="flex gap-3 overflow-x-auto custom-scrollbar py-1">
                  {followedArtists.map((fa) => (
                    <button
                      key={fa.artistId}
                      onClick={() =>
                        handleFollowedArtistClick(fa.artistId, fa.nameEn)
                      }
                      className="flex-shrink-0 w-24 flex flex-col items-center gap-1.5"
                    >
                      <img
                        src={fa.imgUrl || "https://placehold.co/64x64"}
                        alt={fa.nameKr}
                        className="w-12 h-12 rounded-full object-cover border border-gray-100 shadow-sm"
                      />
                      <span className="text-[11px] font-medium text-gray-800 text-center line-clamp-2">
                        {fa.nameKr}
                      </span>
                    </button>
                  ))}
                ...
                <div className="flex flex-col items-center justify-center text-center text-xs text-gray-500 gap-2">
                  ...
                  <button
                    onClick={() => navigate("/artist-list")}
                    className="inline-flex items-center justify-center px-3 py-1.5 rounded-full bg-purple-600 text-white text-[11px] font-semibold shadow-sm"
                  >
                    아티스트 팔로우 하러가기
                  </button>
                </div>
