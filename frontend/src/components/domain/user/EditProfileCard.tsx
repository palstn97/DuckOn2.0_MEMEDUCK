...
// 언어 옵션 정의 (라벨은 fallback, 실제 렌더링은 i18n)
const LANGUAGE_OPTIONS = [
  { value: "ko", label: "한국어" },
  { value: "en", label: "English" },
  { value: "ja", label: "日本語" },
...
const EditProfileCard = ({
    ...
    if (file) {
      ...
      if (file.size > MAX_SIZE) {
        setErrorDialog({
          open: true,
          title: t(
            "mypage.profile.error.fileTooLargeTitle",
            "파일 용량 초과",
          ),
          message: [
            t(
              "mypage.profile.error.fileTooLarge.line1",
              "이미지 용량이 너무 큽니다.",
            ),
            "",
            `${t(
              "mypage.profile.error.fileTooLarge.currentSizeLabel",
              "파일 크기",
            )}: ${fileSizeMB}MB`,
            `${t(
              "mypage.profile.error.fileTooLarge.maxSizeLabel",
              "최대 허용",
            )}: ${maxSizeMB}MB`,
            "",
            t(
              "mypage.profile.error.fileTooLarge.lineLast",
              "더 작은 이미지를 선택해주세요.",
            ),
          ].join("\n"),
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const incomingImg = (refreshed as any)?.imgUrl;
      const safeImg =
        incomingImg && String(incomingImg).trim() !== ""
          ? incomingImg
          : prevImg ?? incomingImg;
      ...
      setErrorDialog({
        open: true,
        title: t(
          "mypage.profile.error.updateFailTitle",
          "프로필 수정 실패",
        ),
        message: [
          t(
            "mypage.profile.error.updateFail.line1",
            "프로필 수정 중 오류가 발생했습니다.",
          ),
          t(
            "mypage.profile.error.updateFail.line2",
            "다시 시도해주세요.",
          ),
        ].join("\n"),
      });
  ...
  return (
    <>
      <div className="bg-white rounded-xl px-4 sm:px-8 py-6 mb-10 w-full max-w-[880px] mx-auto shadow-sm">
        {/* 헤더 */}
        <div className="flex justify-between items-center mb-6">
          <UIText as="h1" id="mypage.profile.edit" className="text-lg font-bold">
            프로필 수정
          </UIText>
          <div className="space-x-2">
            <button
              ...
              onClick={handleSubmit}
            >
              <UIText id="common.save">저장</UIText>
            </button>
            <button
              className="text-sm text-gray-500 font-medium hover:underline"
              onClick={onCancel}
          ...
          <div className="flex flex-col items-center w-full sm:w-32 shrink-0">
            ...
            {showImageOptions && (
              <div className="mt-2 space-y-1 text-sm">
                ...
                >
                  <UIText id="mypage.profile.image.change">
                    변경하기
                  </UIText>
                </button>
                {user.imgUrl && user.imgUrl !== DEFAULT_IMG && (
                  <button
                    ...
                    className="text-gray-500 hover:underline"
                  >
                    <UIText id="mypage.profile.image.resetToDefault">
                      기본 이미지로 변경
                    </UIText>
                  </button>
                )}
            ...
            <div className="mt-4 flex gap-10 sm:gap-6 text-center">
              <div>
                ...
                <div className="text-xs text-gray-500">
                  <UIText id="common.follower">팔로워</UIText>
                </div>
              </div>
              <div>
                <div className="text-lg font-bold">
                ...
                <div className="text-xs text-gray-500">
                  <UIText id="common.following">팔로잉</UIText>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="min-w-0 flex-1 text-sm grid gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[8rem,1fr] items-center gap-2 sm:gap-3">
              <label className="text-gray-500 font-medium">
                <UIText id="mypage.profile.email">이메일</UIText>
              </label>
              <div className="min-w-0">
                <input
                  value={user.email}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[8rem,1fr] items-center gap-2 sm:gap-3">
              <label className="text-gray-500 font-medium">
                <UIText id="mypage.profile.userId">아이디</UIText>
              </label>
              <div className="min-w-0">
                <input
                  value={user.userId}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[8rem,1fr] items-center gap-2 sm:gap-3">
              ...
              >
                <UIText id="mypage.profile.nickname">닉네임</UIText>
              </label>
              <div className="min-w-0">
                <input
                  id="nickname"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[8rem,1fr] items-center gap-2 sm:gap-3">
              <label
                ...
                className="text-gray-500 font-medium"
              >
                <UIText id="mypage.profile.language">언어</UIText>
              </label>
              <div className="min-w-0 relative">
                <select
                  id="language"
