...
//     setBlockedUserIds((prev) => new Set(prev).add(userId));
//   };
//   const visibleMessages = messages.filter((m) => !blockedUserIds.has(m.senderId));
...
//     setTimeout(() => setIsPlaylistUpdating(false), 1500);
//   };
...
//   /* ---------------- 플레이리스트: 순서 변경/삭제 → POST /playlist ---------------- */
//   const handleReorderPlaylist = async (from: number, to: number) => {
//     if (!roomId || !room || !isHost) return;

//     const curList = Array.isArray(room.playlist) ? [...room.playlist] : [];
//     const n = curList.length;
//     if (n <= 1) return;
//     if (from < 0 || from >= n || to < 0 || to >= n || from === to) return;

//     const [moved] = curList.splice(from, 1);
//     curList.splice(to, 0, moved);

//     // currentVideoIndex 보정(안전)
...
//             ...prev,
//             playlist: curList,
//             currentVideoIndex: Math.max(0, Math.min(curList.length - 1, nextIndex)),
//             currentTime: 0,
//             lastUpdated: Date.now(),
//           }
//         : prev
...
//     try {
//       await updatePlaylist(roomId, {
//         playlist: curList,
//         nextIndex: Math.max(0, Math.min(curList.length - 1, nextIndex)),
//       });
//     } catch (e) {
//       console.error("플레이리스트 순서 변경 실패:", e);
//       setRoom(snapshot);
//     }
//   };
...
//     else if (index === nextIndex) nextIndex = Math.min(nextIndex, curList.length - 1);

//     const clamped = Math.max(0, Math.min(curList.length - 1, nextIndex));
...
//     try {
//       await updatePlaylist(roomId, { playlist: curList, nextIndex: clamped });
//     } catch (e) {
...
//   /* ---------------- 방 상태 구독(SYNC_STATE/ROOM_UPDATE/ROOM_DELETED) ---------------- */
//   useEffect(() => {
//     if (!myUser || isQuizModalOpen || !roomId) return;
...
//       sub = syncClient.subscribe(`/topic/room/${roomId}`, (message: IMessage) => {
//         try {
//           const evt = JSON.parse(message.body) as LiveRoomSyncDTO;
//           const t = evt?.eventType;
...
//                   title: evt.title ?? prev.title,
//                   hostNickname: evt.hostNickname ?? prev.hostNickname,
//                   playlist: Array.isArray(evt.playlist)
//                     ? evt.playlist
//                     : prev.playlist,
//                   currentVideoIndex:
//                     typeof evt.currentVideoIndex === "number"
...
//         {/* 오른쪽: 사이드바 */}
//         <aside className="w-full md:w-80 bg-gray-800 flex flex-col border-t md:border-t-0 md:border-l border-gray-700 max-h-[44svh] md:max-h-none overflow-hidden flex-shrink-0">
//           {/* 탭 버튼 */}
//           <div className="flex flex-shrink-0 border-b border-t md:border-t-0 border-gray-700">
//             <button
//               onClick={() => setActiveTab("chat")}
...
// src/pages/live/index.tsx (LiveRoomPage)
...
//   // 🔒 현재 roomId/artistId를 안전하게 참조하기 위한 ref
//   const roomIdRef = useRef<string | undefined>(roomId);
//   const artistIdRef = useRef<number | undefined>(undefined);
...
//   // 🔄 roomId / artistId 변동을 ref에 반영 (이펙트 재등록 없이 최신값 사용)
//   useEffect(() => {
//     roomIdRef.current = roomId;
//   }, [roomId]);
//   useEffect(() => {
//     artistIdRef.current = resolvedArtistId;
//   }, [resolvedArtistId]);
...
//     const curList = Array.isArray(room.playlist) ? [...room.playlist] : [];
//     if (curList.length <= 1) {
//       return; // 서버 정책상 빈 배열 금지 → 마지막 1개는 삭제 불가
//     }
//     if (index < 0 || index >= curList.length) return;
...
//   /* ---------------- 언로드/뒤로가기 시 정리 ---------------- */
//   // ✅ 리스너는 한 번만 등록, cleanup에서 onPageHide를 호출하지 않음!
//   useEffect(() => {
//     const onPageHide = () => {
//       const rid = roomIdRef.current;
//       const aid = artistIdRef.current;
//       if (!rid || !aid) return;
//       if (!joinedRef.current) return;
//       if (isRefreshingRef.current || wsHandoverRef.current) return;
//       if (leavingRef.current) return;
...
//       if (isHostRef.current) {
//         fireAndForget(`/rooms/${rid}?artistId=${aid}`, "DELETE");
//       } else {
//         fireAndForget(`/rooms/${rid}/exit?artistId=${aid}`, "POST");
//       }
//     };

//     // 탭 닫기/새로고침/다른 사이트로 이동
//     window.addEventListener("pagehide", onPageHide);
//     window.addEventListener("beforeunload", onPageHide);

//     // 브라우저 뒤로가기(같은 탭 내 SPA 이동)는 언로드가 없으므로 popstate 보조
//     const onPopState = () => {
//       const rid = roomIdRef.current;
//       const aid = artistIdRef.current;
//       if (!rid || !aid) return;
...
//       // ❌ cleanup에서 onPageHide()를 호출하지 않습니다.
//     };
//   }, []); // 한 번만 등록
