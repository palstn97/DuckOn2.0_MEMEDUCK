...
const PasswordConfirm = ({ isOpen, onClose, onConfirm }: PasswordConfirmProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!isValid) setError("비밀번호가 일치하지 않습니다.");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl p-6 w-90 text-center shadow-xl">
        ...
        <div className="flex justify-between gap-2">
          ...
          <button
            className={`w-1/2 py-2 text-sm rounded text-white ${password.trim() ? "bg-purple-500 hover:bg-purple-600" : "bg-purple-300 cursor-not-allowed"}`}
