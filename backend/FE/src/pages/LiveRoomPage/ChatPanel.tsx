...
import { useState, useEffect, useRef, useMemo } from "react";
import { Send, MoreVertical, UserX, LockKeyhole, Languages } from "lucide-react";
import { Popover, Transition } from "@headlessui/react";
import { useUserStore } from "../../store/useUserStore";
import type { ChatMessage } from "../../types/chat";
import { translateMessage } from "../../api/translateService";
...
const ChatPanel = ({ messages, sendMessage, onBlockUser }: ChatPanelProps) => {
  ...
  // 마운트 시마다 1회 서버에서 강제 동기화(이전 blockedSet 여부 무시)
  const syncedOnceRef = useRef(false);
  useEffect(() => {
    if (!myUser || syncedOnceRef.current) return;
  ...
  }, [myUser, setBlockedList]);

  // 차단 기준으로 메시지 필터링 (Set 내부 변경까지 추적 + id 문자열 정규화)
  const visibleMessages = useMemo(() => {
    const bs = blockedSet instanceof Set ? blockedSet : new Set<string>();
    return messages.filter((m) => {
      if (m.chatType === "ENTER") return true; // 시스템 메시지는 항상 표시
      return !bs.has(String(m.senderId));
    });
  }, [messages, blockedSet, blockedSet.size]);
  ...
  const confirmBlock = async () => {
    if (!blockConfirm.user) return;
    const id = String(blockConfirm.user.id); // ✅ 정규화
      ...
      <div className="flex flex-col h-full bg-gray-800 text-white">
        <div className="flex-1 space-y-4 overflow-y-auto overscroll-contain p-4 min-h-0">
          {visibleMessages.map((msg, index) => {
            ...
            const uniqueKey = `${msg.senderId}-${msg.sentAt || index}`;
            const isMyMessage = String(msg.senderId) === String(myUser?.userId);
            const t = translations[uniqueKey];
            const contentToShow =
              t?.showingTranslated && t?.text ? t.text : msg.content;
