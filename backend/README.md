아래 내용을 그대로 `README.md`로 바꿔 넣으면 돼. (최종본)
...
**덕온(Duck-On)** 은 유튜브 영상을 **동시에 시청**하고 **실시간 채팅**으로 소통하는 팬 커뮤니티 플랫폼입니다.
...
* `exec/db/duckon_prod_artist.sql` — **artist 데이터 덤프**(DDL 제외, INSERT만)
